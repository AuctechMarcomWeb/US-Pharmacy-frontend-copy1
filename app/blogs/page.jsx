...
const metadata = {
  title: "Medicines Information & Availability Inquiry | US Pharmacy",
  description: "Explore medicine information, dosage details and submit availability inquiries. All requests are subject to prescription verification and manual review.",
  keywords: [
  ...
  return (
    <div>
      <button
        ...
        type="button"
        onClick={handleOpen}
        className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border-2 text-sm font-semibold transition-all bg-white ${open
            ...
            <div className="max-h-52 overflow-y-auto">
              {packs.map((pack, idx) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 text-sm transition-colors ${idx < packs.length - 1 ? "border-b border-slate-100" : ""
                    } ${activePack === idx
...
const normalize = (p) => {
  ...
  return {
    id: p._id || p.id,
    name: p.title || p.name || "Unnamed Medicine",
...
export default function BlogPage() {
  ...
  const handleAddToCart = (product, qty, selPack) => {
    ...
    toast.success(`${product.name} ×${qty} added to inquiry list`);
      ...
      <div className="min-h-screen bg-[#f5f7fb] px-4 py-8 sm:py-10">
        ...
        <div className="max-w-4xl mx-auto text-center mb-7 sm:mb-8">
          <h1 className="text-3xl sm:text-3xl font-extrabold text-slate-900 tracking-tight text-[#166534]">
            Medicines Information & Availability Inquiry
          </h1>
          <p className="text-sm text-slate-400 mt-1.5">
            Browse medicine information and submit availability inquiries. Prescription verification may be required.
          </p>
        </div>
                        ...
                        <div className="flex gap-1.5 overflow-x-auto pb-1">
                          {p.images.map((img, idx) => (
                            <img
                              ...
                              className={`w-10 h-10 object-cover rounded-lg shrink-0 cursor-pointer transition-all border-2 ${activeImg === idx
                    ...
                    <div className="flex-1 p-4 sm:p-5 lg:p-6 flex flex-col">
                      ...
                      <div className="flex items-center gap-3 mt-auto">
                        ...
                        <button
                          ...
                          className={`flex-1 h-9 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all ${!selInStock
